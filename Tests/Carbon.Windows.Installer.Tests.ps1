...
Describe ('Carbon.Windows.Installer.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.Windows.Installer' -Exists
    }
}

Describe ('Carbon.Windows.Installer.command verbs') {
...
Describe ('Carbon.Windows.Installer.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.Windows.Installer' -CommandType Function,Cmdlet,Filter))
        {
